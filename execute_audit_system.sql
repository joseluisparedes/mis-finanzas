-- EJECUTAR SISTEMA COMPLETO DE AUDITORÍA EN SUPABASE
...
DROP TRIGGER IF EXISTS audit_income_trigger ON income;
...
-- ================================================================
-- PASO 5: FUNCIONES DE CONSULTA PARA DASHBOARD
-- ================================================================
...
BEGIN
  ...
  SELECT 
    ...
    up.full_name, up.first_name, up.avatar_url,
    ...
    COALESCE(es.total, 0), COALESCE(ins.total, 0), COALESCE(bs.total, 0),
    als.last_activity, COALESCE(als.total_actions, 0),
    COALESCE(ss.current_sessions, 0), ss.last_ip, ss.last_user_agent
  FROM auth.users u
  LEFT JOIN user_profiles up ON u.id = up.user_id
  LEFT JOIN user_subscriptions us ON u.id = us.user_id
  LEFT JOIN account_status acs ON u.id = acs.user_id
  LEFT JOIN (SELECT user_id, COUNT(*) as total FROM expenses GROUP BY user_id) es ON u.id = es.user_id
  LEFT JOIN (SELECT user_id, COUNT(*) as total FROM income GROUP BY user_id) ins ON u.id = ins.user_id
  LEFT JOIN (SELECT user_id, COUNT(*) as total FROM budgets GROUP BY user_id) bs ON u.id = bs.user_id
  LEFT JOIN (
  ...
  LEFT JOIN (
    SELECT user_id, COUNT(*) FILTER (WHERE is_active = TRUE) as current_sessions,
           MAX(ip_address) as last_ip, MAX(user_agent) as last_user_agent
...
BEGIN
  ...
  SELECT 
    u.id, u.email, u.created_at, u.last_sign_in_at,
    up.full_name, us.subscription_type, COALESCE(acs.status, 'active'),
    COALESCE(es.total, 0), COALESCE(ins.total, 0), COALESCE(bs.total, 0),
  ...
  LEFT JOIN user_subscriptions us ON u.id = us.user_id
  LEFT JOIN account_status acs ON u.id = acs.user_id
  LEFT JOIN (SELECT user_id, COUNT(*) as total FROM expenses WHERE user_id = p_user_id GROUP BY user_id) es ON u.id = es.user_id
  LEFT JOIN (SELECT user_id, COUNT(*) as total FROM income WHERE user_id = p_user_id GROUP BY user_id) ins ON u.id = ins.user_id
  LEFT JOIN (SELECT user_id, COUNT(*) as total FROM budgets WHERE user_id = p_user_id GROUP BY user_id) bs ON u.id = bs.user_id
  LEFT JOIN (
    SELECT user_id, MAX(created_at) as last_activity, COUNT(*) as total_actions
...
-- Exportar datos de usuario completos
...
BEGIN
  ...
  -- Gastos
  RETURN QUERY
  SELECT 'expenses'::TEXT, jsonb_agg(row_to_json(e.*))
  FROM expenses e WHERE e.user_id = p_user_id;

  -- Ingresos  
  RETURN QUERY
  SELECT 'income'::TEXT, jsonb_agg(row_to_json(i.*))
  FROM income i WHERE i.user_id = p_user_id;

  -- Presupuestos
  RETURN QUERY
  SELECT 'budgets'::TEXT, jsonb_agg(row_to_json(b.*))
  FROM budgets b WHERE b.user_id = p_user_id;

  -- Historial de auditoría (últimos 1000)
  RETURN QUERY
  SELECT 'audit_history'::TEXT, jsonb_agg(
    jsonb_build_object(
...
BEGIN
  ...
  SELECT 'premium_users'::TEXT, COUNT(*)::BIGINT, 
         ROUND((COUNT(*)::DECIMAL / (SELECT COUNT(*) FROM user_subscriptions) * 100), 2)
  FROM user_subscriptions WHERE subscription_type LIKE 'premium%'
  
  UNION ALL
  SELECT 'suspended_accounts'::TEXT, COUNT(*)::BIGINT,
         ROUND((COUNT(*)::DECIMAL / (SELECT COUNT(*) FROM user_subscriptions) * 100), 2)
...
SELECT log_audit_event(
  ...
  'Sistema completo de auditoría desplegado exitosamente',
  'system',
